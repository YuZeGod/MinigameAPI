package net.afyer.minigameapi.database;
...
public class DataBase
{
    ...
    {
        dataBaseCore = core;
        logger = MinigameAPI.getInstance().getLogger();
    }
    ...
    public static DataBase create(ConfigurationSection dbConfig)
    {
        ...
        switch (type)
        {
            ...
            default:
                return new DataBase(new SQLiteCore(MinigameAPI.getInstance(), dbConfig));
        }
    }
    ...
    public boolean copyTo(DataBaseCore db)
    {
        try
        {
            ...
            ResultSet rs = dataBaseCore.getConnection().getMetaData()
            ...
            {
                logger.info("开始复制源数据库中的表 " + table + " ...");
                if (!table.toLowerCase().startsWith("sqlite_autoindex_"))
                {
                    MinigameAPI.getInstance().getLogger()
                            .info("清空目标数据库中的表 " + table + " ...");
                    ...
                    logger.info("数据表 " + table + " 复制完成 共 " + n + " 条记录...");
                    s += n;
                    ps.executeBatch();
                    rs.close();
                }
            }
            logger.info("成功从 " + src + " 复制 " + s + " 条数据到 " + des + " ...");
    ...
    {
        String sql = "SELECT " + fields + " FROM " + tableName + " WHERE " + selConditions
                .toWhereString() + " limit 1";
        try
        {
            ResultSet dbresult = dataBaseCore.executeQuery(sql);
            if (dbresult.next())
